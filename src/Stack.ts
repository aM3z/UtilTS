...
export class Stack<E>  {
   ...
   * otherwise.
   */
  public empty(): boolean { return this._elementData.length === 0; }
   ...
   * the stack.
   */
  public search(o: Object): number {
    // position default to -1
    let position: number = -1;
    // iterate through stack from top to bottom
    loop: {
      for (let i: number = this._elementData.length - 1; i >= 0; i--) {
